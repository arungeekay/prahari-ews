...
Working-capital conduct signals that the bank's own core-banking APIs expose are generated
causally as well (IDBI API 441 drawing-power history, API 362 liens, stock-statement
submission, and the anchor-attributable inflow that in production is read from statement
credits grouped by payer). Officer notes are free text composed from templates with slots.
...
Key invariants preserved throughout:
    ...
    * Term loans carry no drawing power / utilisation-creep channel; stress shows in cash-flow.
"""
...
# ---- loan-officer note templates: compositional, keyed to account state (BUILD_SPEC §3.2) ----
# Slots: {buyer} counterparty, {n}/{m} small counts, {days} a day count. Themes a banker would
# recognise are embedded in the wording (receivables, labour, demand, input cost, promoter
# conduct, legal, stock, capacity) so a text scorer has something real to extract.
_BUYERS = ["the anchor buyer", "a Pune OEM", "the Surat distributor", "two Ludhiana traders",
           "the main distributor", "an export client", "a government department", "the Chennai buyer",
           "the Rajkot dealer network", "a Tier-1 auto customer"]
_NOTE_TEMPLATES = {
    "growing": [
        "Capacity expansion underway; {n} new machines commissioned this quarter.",
        "Order inflow strong; promoter upbeat on demand from {buyer}.",
        "Unit visit: additional shift running, hiring {n} operators.",
        "Stock and receivables verified; turnover running ahead of projections.",
        "Enquired about limit enhancement to fund confirmed orders from {buyer}.",
    ],
    "healthy": [
        "Unit visit: godown well stocked, operations normal.",
        "Stock and receivables statement verified; no adverse observations.",
        "Routine review: account conduct satisfactory, drawings within DP.",
        "Promoter cooperative; order book healthy for the next {n} months.",
        "Telephonic follow-up: production normal, receivables current.",
    ],
    "warning": [
        "Promoter mentions receivables from {buyer} delayed by {days} days.",
        "Utilisation edging up; promoter enquired about a temporary overlimit.",
        "Slight slowdown in dispatches; promoter expects festival demand to help.",
        "Stock statement submitted late; debtors ageing beyond {days} days rising.",
        "Input costs up; promoter negotiating longer credit period with suppliers.",
    ],
    "distress": [
        "Unit visit: reduced activity, {n} of {m} machines idle.",
        "Promoter evasive about receivables and creditor pressure from {buyer}.",
        "Cheque presented late; promoter cites payment delays from {buyer}.",
        "Stock levels lower than book value; debtors over {days} days now material.",
        "Workers report wages delayed by {days} days; EPFO remittance pending.",
        "Promoter seeking additional finance elsewhere to bridge working capital.",
        "Statutory dues overdue; GST notice received for a {days} day filing delay.",
    ],
    "severe": [
        "Unit running a single shift; promoter unable to meet interest this month.",
        "Legal notice received from a supplier; receivables from {buyer} disputed.",
        "Physical stock materially below statement; promoter under acute cash pressure.",
        "Promoter not reachable for {days} days; branch visit found the unit shut for the week.",
    ],
    "shock": [
        "Sudden setback: fire reported at unit, operations halted.",
        "Key buyer {buyer} exited abruptly; large order book lost overnight.",
        "Promoter dispute disclosed; business operations disrupted.",
        "Flood damage to godown; insurance claim lodged, production stopped.",
    ],
    "slow": [
        "Chronic slow collections from {buyer}; utilisation persistently high.",
        "Margins thin; promoter servicing interest with some delay but regular.",
        "Unit visit: operations steady at reduced scale, no new orders in hand.",
        "Old receivables from {buyer} still outstanding; promoter following up.",
    ],
    "plateau": [
        "Stress persists but stable; promoter servicing dues on time.",
        "Receivables position unchanged; no fresh deterioration observed.",
        "Utilisation high but within limit; interest serviced regularly.",
    ],
    "fraud": [
        "Turnover growth reported but floor activity appears modest.",
        "Routine review: promoter reports rising sales; physical verification pending.",
        "Books show strong turnover; electricity bill unchanged from last year.",
        "Debtors list includes {n} new parties; balance confirmations awaited.",
    ],
}
...
def _utilisation(rng, row) -> np.ndarray:
    """CC/OD: drawn / sanctioned, with utilisation creep under stress. Term: amortising
    outstanding / sanctioned - a bookkeeping ratio that carries NO stress information."""
    traj = row.health_trajectory
    sev = float(row.severity)
    if row.loan_type == "term":
        r0 = rng.uniform(*C.TERM_LOAN["initial_outstanding"])
        tenor = int(rng.integers(*C.TERM_LOAN["tenor_months"]))
        util = r0 - (r0 / tenor) * _MONTHS + rng.normal(0, C.TERM_LOAN["noise"], _M)
        return np.clip(util, 0.05, 0.99)
...
def _drawing_power(rng, row) -> np.ndarray:
    """Drawing power as a fraction of sanction (IDBI API 441). Thins as stock and book debts
    shrink under stress. Zero for term loans (not applicable)."""
    if row.loan_type == "term":
        return np.zeros(_M)
    P = C.DRAWING_POWER
    traj = row.health_trajectory
    sev = float(row.severity)
    base = rng.uniform(*P["base"])
    dp = np.full(_M, base)
    if traj in _SOFT_DISTRESS:
        s = int(row.traj_start)
        for m in range(_M):
            d = m - s
            if d >= P["distress_start"]:
                dp[m] = max(P["distress_floor"], base - (d - P["distress_start"] + 1) * P["distress_decline_per_month"] * sev)
    elif traj == "sudden_shock":
        s = int(row.traj_start)
        dp[s:] = base * P["shock_factor"]
    elif traj == "slow_decline":
        s = int(row.traj_start)
        for m in range(_M):
            if m >= s:
                dp[m] = max(P["slow_floor"], base - (m - s) * P["slow_decline_per_month"])
    dp = dp + rng.normal(0, P["noise"] * _noise_mult(row), _M)
    return np.clip(dp, 0.30, 1.0)


def _stock_statements(rng, row) -> np.ndarray:
    """1 = monthly stock/book-debt statement submitted, 0 = not submitted (an RBI EWS indicator).
    Always 1 for term loans (no statement obligation)."""
    if row.loan_type == "term":
        return np.ones(_M, np.int64)
    S = C.STOCK_STATEMENT
    traj = row.health_trajectory
    dm = int(row.default_month)
    p = np.full(_M, S["miss_prob_clean"])
    if traj in _SOFT_DISTRESS:
        s = int(row.traj_start)
        for m in range(_M):
            d = m - s
            if d >= 4:
                p[m] = S["miss_prob_distress"]
            if dm >= 0 and 0 <= dm - m <= 4:
                p[m] = S["miss_prob_severe"]
    elif traj == "sudden_shock":
        s = int(row.traj_start)
        p[s:] = S["miss_prob_shock"]
    elif traj == "slow_decline":
        s = int(row.traj_start)
        p[s:] = S["miss_prob_slow"]
    submitted = (rng.random(_M) >= p).astype(np.int64)
    return submitted


def _liens(rng, row) -> tuple[np.ndarray, np.ndarray]:
    """New liens placed on the operating account (IDBI API 362): statutory holds, court orders,
    supplier claims. IDBI's own API note: 'new liens often precede default'."""
    L = C.LIENS
    traj = row.health_trajectory
    dm = int(row.default_month)
    p = np.full(_M, L["clean_prob_per_month"])
    if traj in _SOFT_DISTRESS:
        s = int(row.traj_start)
        for m in range(_M):
            d = m - s
            if d >= L["distress_start"]:
                p[m] = L["distress_prob_per_month"]
            if dm >= 0 and 0 <= dm - m <= 3:
                p[m] = L["severe_prob_per_month"]
    elif traj == "sudden_shock":
        s = int(row.traj_start)
        p[s:] = L["shock_prob_per_month"]
    elif traj == "slow_decline":
        s = int(row.traj_start)
        p[s:] = L["slow_prob_per_month"]
    count = (rng.random(_M) < p).astype(np.int64)
    frac = rng.uniform(*L["amount_frac_of_sanction"], _M)
    amount = np.where(count > 0, frac * float(row.sanctioned_limit), 0.0)
    return count, amount


def _anchor_inflow(rng, row) -> np.ndarray:
    """Monthly inflow attributable to the anchor buyer (0 for non-suppliers). In production this
    is statement credits grouped by payer; here it is generated so the contagion model can
    MEASURE anchor stress instead of assuming it."""
    if not bool(row.is_anchor_supplier):
        return np.zeros(_M)
    base = float(row.anchor_payment)
    inflow = base * (1 + rng.normal(0, 0.05, _M))
    if row.anchor_id == "ANCH1":
        ev = C.CONTAGION["event_month"] + int(row.anchor_lag)
        inflow[ev:] = inflow[ev:] * (1 - C.CONTAGION["payment_cut"])
...
def _note_state(row, m: int) -> str:
    """Which note family applies at month m, from the latent trajectory (causal, no leakage)."""
    traj = row.health_trajectory
    dm = int(row.default_month)
    if traj == "growing":
        return "growing"
    if traj == "fraud_pattern":
        return "fraud"
    if traj == "sudden_shock":
        return "shock" if m >= int(row.traj_start) else "healthy"
    if traj == "slow_decline":
        return "slow" if m >= int(row.traj_start) else "healthy"
    if traj in _SOFT_DISTRESS:
        d = m - int(row.traj_start)
        if d < 0:
            return "healthy"
        if traj == "distress_then_recover" and d >= 8:
            return "plateau"
        if dm >= 0 and 0 <= dm - m <= 3:
            return "severe"
        if d >= 9:
...
_DAYS = (15, 21, 30, 45, 60, 90)


def _compose_note(rng, template: str) -> str:
    r = rng.integers(0, 1_000_000, 4)     # one draw for all slots (fast, deterministic)
    return template.format(
        buyer=_BUYERS[int(r[0] % len(_BUYERS))],
        n=int(2 + r[1] % 4), m=int(6 + r[2] % 6),
        days=_DAYS[int(r[3] % len(_DAYS))],
    )


def _officer_notes(rng, row) -> list[str]:
    """Free-text officer notes, roughly every other month, composed from state-keyed templates."""
    prob = C.NOTES["demo_monthly_prob"] if row.demo else C.NOTES["monthly_prob"]
    notes = [""] * _M
    for m in range(_M):
        if rng.random() >= prob:
            continue
        pool = _NOTE_TEMPLATES[_note_state(row, m)]
        notes[m] = _compose_note(rng, pool[int(rng.integers(len(pool)))])
    return notes
...
def _gen_borrower(rng: np.random.Generator, row, sent_lookup: dict) -> dict:
    ...
    # --- working-capital conduct signals the bank's own APIs expose ---
    drawing_power = _drawing_power(rng, row)
    stock_stmt = _stock_statements(rng, row)
    lien_count, lien_amount = _liens(rng, row)
    anchor_inflow = _anchor_inflow(rng, row)

    # --- officer notes (free text) ---
    notes = _officer_notes(rng, row)
    ...
    if row.demo == "sharma":
        ...
        notes[C.DEMO_MONTH] = "Cheque presented late; promoter evasive about receivables from the anchor buyer."
    ...
    return dict(
        ...
        drawing_power_pct=drawing_power, stock_statement_submitted=stock_stmt,
        lien_count=lien_count, lien_amount=lien_amount, anchor_inflow=anchor_inflow,
...
FLOAT_COLS = ["credits", "debits", "month_end_balance", "limit_utilisation",
              ...
              "drawing_power_pct", "lien_amount", "anchor_inflow"]
INT_COLS = ["cheque_bounces_inward", "cheque_bounces_outward", "gst_filing_delay_days",
            ...
            "stock_statement_submitted", "lien_count"]
COLUMN_ORDER = ["borrower_id", "month_index", "month_date", "credits", "debits", "month_end_balance",
                "cheque_bounces_inward", "cheque_bounces_outward", "limit_utilisation",
                "drawing_power_pct", "stock_statement_submitted", "lien_count", "lien_amount",
                "anchor_inflow",
...
def build_msme_monthly(rng: np.random.Generator, borrowers: pd.DataFrame):
    ...
    for c in FLOAT_COLS:
        data[c] = (np.round(buf[c], 4) if c in ("limit_utilisation", "sector_sentiment", "drawing_power_pct")
                   else np.round(buf[c], 2))
    for c in INT_COLS:
        data[c] = buf[c]
    data["repayment_status"] = repayment_status
    data["officer_note"] = officer_note
    df = pd.DataFrame(data)
    return df[COLUMN_ORDER], sent_df
