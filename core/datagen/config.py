...
CONTAGION = dict(
    ...
    # Elasticity used by the contagion model to turn an OBSERVED decline in anchor inflows into
    # transmitted stress: a sustained 50% payment cut is treated as fully stressing (stress 1.0).
    # This replaces any per-anchor literal; anchor stress is measured from the anchor_inflow
    # series (in production: statement credits grouped by payer).
    inflow_loss_to_stress=2.0,
    # Months of anchor inflow used as the baseline / recent window when measuring the decline.
    stress_baseline_months=12,
    stress_recent_months=3,
)

# --------------------------------------------------------------------------------------
# Working-capital conduct signals the bank's own APIs expose (IDBI API 441 drawing power,
# API 362 liens, stock-statement submission). IDBI's API descriptions name the sanction-to-
# drawing-power gap and new liens as leading default indicators; the generator encodes them
# causally so the model can learn them honestly. CC/OD only - term loans carry no drawing power.
# --------------------------------------------------------------------------------------
DRAWING_POWER = dict(
    base=(0.88, 0.98),                # healthy DP as a fraction of sanction
    distress_start=2,                 # d >= 2: stock and book-debt margins start thinning
    distress_decline_per_month=0.025,
    distress_floor=0.45,
    slow_decline_per_month=0.008, slow_floor=0.72,
    shock_factor=0.55,                # DP collapses with the shock
    noise=0.012,
)
STOCK_STATEMENT = dict(
    miss_prob_distress=0.35,          # per month from d >= 4: statement not submitted
    miss_prob_severe=0.60,            # in the final 4 months before default
    miss_prob_shock=0.60,
    miss_prob_slow=0.08,
    miss_prob_clean=0.02,
)
LIENS = dict(
    distress_start=6, distress_prob_per_month=0.12,   # statutory / court liens appear late
    severe_prob_per_month=0.25,
    shock_prob_per_month=0.45,
    slow_prob_per_month=0.02,
    clean_prob_per_month=0.004,
    amount_frac_of_sanction=(0.02, 0.15),
)
# Term-loan "utilisation" is an amortising outstanding-to-sanction ratio and carries no stress
# information (the feature pipeline masks it for term loans and reads stress from cash-flow).
TERM_LOAN = dict(initial_outstanding=(0.55, 0.95), tenor_months=(36, 84), noise=0.004)

# Officer notes: free-text observations, roughly every other month, compositional templates.
NOTES = dict(monthly_prob=0.55, demo_monthly_prob=0.70)
